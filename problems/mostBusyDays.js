...
function getMostBusyDates(reservations) {
    ...
    const reservationDates = Array(latestDate - earliestDate + 1).fill().map((_, i) => i + earliestDate)

    // Declare an array with counters per reservation day, initiated with zeros
    const totalReservationsPerDate =  reservationDates.reduce((prev, curr) => ({ ...prev, [curr]: 0 }), {})
    ...
    for (let i=7; i<reservationDates.length; i++) {
        ...
        const average = Object.values(totalReservationsPerDate).slice(i-7, i).reduce((prev, curr) => prev+curr, 0)/7
