...
function getHotelReviewScores(positiveKeyWords, negativeKeyWords, hotelIds, reviews, k) {
    ...
    const hotelScores = hotelIds.reduce((prev, curr) => ({ ...prev, [curr]: 0 }), {})
    ...
    // Sort hotel scores
    const sortedScores = Object.keys(hotelScores).map(x => Number(x)).sort((a,b) => hotelScores[a] === hotelScores[b] ? a - b : hotelScores[b] - hotelScores[a])
